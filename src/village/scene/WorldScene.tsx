...
function World(props:Props&{motion:boolean}){
...
export default function WorldScene(props:Props){
 const [motion,setMotion]=useState(true),[reduced,setReduced]=useState(false),[list,setList]=useState(false),[visible,setVisible]=useState(true),[lost,setLost]=useState(false);const host=useRef<HTMLDivElement>(null);
 ...
 {props.selected!==null&&props.v.plots[props.selected]&&<div className="scene-selection" role="status"><strong>{BUILD[props.v.plots[props.selected]!].name}</strong><small>{buildingStatus(props.v,props.v.plots[props.selected]!,props.selected).label}</small></div>}
