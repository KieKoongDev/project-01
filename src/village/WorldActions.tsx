...
import {RotateCw,ArrowUpRight,FlaskConical,Move,UserPlus,Users,X} from 'lucide-react';
...
export function WorldActions({v,tile,dispatch,onTab,onRecipe,onMove,onClose}:{v:Village;tile:number;dispatch:(a:Action)=>void;onTab:(tab:string)=>void;onRecipe:(r:Recipe)=>void;onMove:()=>void;onClose:()=>void}){
 ...
 <button aria-label="หมุนอาคาร 90 องศา" onClick={()=>dispatch({type:'rotate',tile})}><RotateCw size={18}/></button><button aria-label="ย้ายอาคารฟรี" onClick={onMove}><Move size={18}/><span>ย้าย</span></button><button aria-label="รายละเอียดอาคาร" onClick={()=>onTab('object')}><ArrowUpRight size={18}/><span>เพิ่มเติม</span></button>
 </div></section>;
}
