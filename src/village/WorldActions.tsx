...
export function WorldActions({v,tile,dispatch,onTab,onRecipe,onMove,onClose}:{v:Village;tile:number;dispatch:(a:Action)=>void;onTab:(tab:string)=>void;onRecipe:(r:Recipe)=>void;onMove:()=>void;onClose:()=>void}){
 ...
 {b==='shelter'?<button disabled={!!recruitReason} title={recruitReason??'ใช้อาหาร 4'} onClick={()=>dispatch({type:'recruit'})}><UserPlus size={18}/>{recruitReason??'รับเพื่อน · อาหาร 4'}</button>:b==='kiln'&&!v.pots?<button disabled={v.known.includes('pottery')&&!!potteryReason} onClick={()=>v.known.includes('pottery')?dispatch({type:'produce',item:'pottery'}):onRecipe('pottery')}><FlaskConical size={18}/>{!v.known.includes('pottery')?'เรียนรู้ภาชนะ':potteryReason??'ทำภาชนะ · ดิน 2 ไม้ 1'}</button>:<button onClick={()=>onTab(status.tab)}><Users size={18}/>{b==='farm'||b==='lumber'?'จัดคนทำงาน':'ดูการใช้งาน'}</button>}
