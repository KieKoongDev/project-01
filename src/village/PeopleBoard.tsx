...
import {Briefcase,GripVertical,UserRound,Users} from 'lucide-react';
...
export function PeopleBoard({v,dispatch}:{v:Village;dispatch:(a:Action)=>void}){
 ...
 return <><div className="people-roster" aria-label="เลือกชาวบ้านเพื่อมอบหมายงาน">{Array.from({length:v.people},(_,i)=>{const job=v.assignments[i];return <button key={i} className={'person-card '+(selected===i?'selected ':'')+(dragging===i?'dragging':'')} aria-pressed={selected===i} onClick={()=>setSelected(selected===i?null:i)} onPointerDown={e=>{if(e.button!==0)return;didDrag.current=false;start.current={person:i,x:e.clientX,y:e.clientY};setSelected(i);}}><span className="person-avatar"><UserRound size={23}/></span><span><strong>{v.names[i]}</strong><small>{job?'ผู้เชี่ยวชาญ'+JOB_NAMES[job]:'ว่าง · เลือกงานได้'}</small></span><GripVertical size={18}/></button>;})}</div>
 ...
 <div className="job-board">{JOBS.map(j=>{const unavailable=j==='farm'&&!v.plots.includes('farm');return <button key={j} data-job={j} className="job-card" disabled={unavailable} onClick={()=>assign(j)}><ItemArt kind={j==='forage'?'food':j==='farm'?'farm':j}/><span><strong>{JOB_NAMES[j]}</strong><small>{output(v,j)}</small><em>{v.jobs[j]} คน · ทำต่อขณะ AFK</em></span></button>;})}</div>
